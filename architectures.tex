...
\licenseframe{github.com/severin-lemaignan/lecture-hri-architectures}
        ...
        \begin{column}{0.5\linewidth}
            ...
            \begin{itemize}
                \item a map with the important landmarks like {\tt fridge}
                    \begin{center}
                        \includegraphics[width=0.5\linewidth]{figs/map}
                    \end{center}
            ...
            \begin{center}
                \resizebox{\linewidth}{!}{
                    \begin{tikzpicture}[>=latex,every node/.style={draw,font=\tt}]
                        ...
                        \node at (0.5,2) {robot\_navigation};
                        \node at (2.5,4) {door\_opening};
                        \node at (-0.7,4) {speech\_synthesis};
                        \node at (0,0) {natural\_language\_processor};
                        \node at (3,1) {person\_detector};
                        \node at (2.7,3) {cat\_detector};
                        \node at (-0.5,1) {route\_planner};
                        \node at (-0.3,3) {object\_picking};
                    \end{tikzpicture}
                }
            \end{center}
        \end{column}
...
\begin{frame}{Combining behaviours: example}
    ...
    \only<1>{
        ...
        \begin{itemize}
            \item {\bf Follow a human}
                \begin{itemize}
                    \item Sensor: camera
                    \item Behaviour: follow a human seen in camera
                \end{itemize}
        ...
        \source{https://www.clear.rice.edu/engi128/Handouts/Lec17-Robotics.pdf}{example
        inspired by Rice University ENGI128}
    }
    \only<2->{
        \begin{center}
...
{
    \paper{Brooks {\bf A robust layered control system for a mobile robot}, 1986}
...
\begin{frame}{Example: the MIT Genghis robot}
    \video[0.75]{0.7\pagewidth}{videos/genghis-short2.mov}
\end{frame}
...
\begin{frame}{Task planning}
    ...
    \centering
        ...
        \includegraphics[width=0.8\linewidth]{cleantable/clean-table-situation}

        \only<1>{
            {\bf Goal:} (collaboratively) put the books into the pink bin.

        \small
        \begin{itemize}
            \item The bin is only accessible to the human
            \item Three books: a black one and a grey one (only accessible to
                robot), a white one (only accessible to the human).
        \end{itemize}
    }
    ...
    \resizebox{0.9\linewidth}{!}{%
        \begin{tikzpicture}[
                >=latex,
            robot/.style={fill=hriSec2Comp!20},
            ...
            every node/.style={ draw, 
                    ...
                    \node<3>[font=\rm\Huge, draw=none, fill=none, rotate=90] at (-3, 0) {Human\\actions};
            \node<2->[font=\rm\Huge, draw=none, fill=none, rotate=90] at (-3, 4.5) {Robot\\actions};

            \coordinate (h1) at (0,0);

            \node<3> at (h1) (hh1) {\bf TAKE\\WHITE\_BOOK\\TABLE};
            \node<3>[right=of hh1] (h2) {\bf PUT\\WHITE\_BOOK\\PINK\_BIN } edge (hh1);

            \node<2->[robot, above=2.5 of h1] (r1) {\bf TAKE\\BLACK\_BOOK\\TABLE };
            \node<2->[robot, right=of r1] (r2) {\bf PUT\_REACH\\BLACK\_BOOK\\TABLE } edge (r1);
            \node<2->[robot, right=of r2] (r3) {\bf TAKE\\GREY\_BOOK\\TABLE } edge (r2);
            \node<2->[robot, right=of r3] (r4) {\bf PUT\_REACH\\GREY\_BOOK\\TABLE} edge (r3);

            \node<3> at (r3 |- h1) (h3) {\bf TAKE\\BLACK\_BOOK\\TABLE} edge (h2) edge (r2);
            \node<3>[right=of h3] (h4) {\bf PUT\\BLACK\_BOOK\\PINK\_BIN} edge (h3);
            \node<3>[right=of h4] (h5) {\bf TAKE\\GREY\_BOOK\\TABLE} edge (h4) edge (r4);
            \node<3>[right=of h5] (h6) {\bf PUT\\GREY\_BOOK\\PINK\_BIN} edge (h5);
...
\videoframe[0.7]{videos/clean-table.webm?start=20}
...
\begin{frame}{Full social \& autonomous interaction: one example}
    ...
    \begin{center}
     ...
     \begin{columns}
         ...
         \node[fill=white,align=left] at (cam1 -| kbr) (kb1) {\stmt{BOOK isVisible true}\\
                                                    \stmt{BOOK isReachable true}\\
                                                    \stmt{BOOK isOn TABLE}\\
                                                    \stmt{BIN isVisible true}\\
                                                    \stmt{BIN isReachable false}};

         \node[fill=white,align=left] at (cam1 -| kbh) {\stmt{BOOK isVisible true}\\
                                                    \stmt{BOOK isReachable false}\\
                                                    \stmt{BOOK isOn TABLE}\\
                                                    \stmt{BIN isVisible true}\\
                                                    \stmt{BIN isReachable true}};

         \node[fill=white,align=left] at (cam2 -| kbr) (kb2) {\stmt{ROBOT hasInHand BOOK}};


         \node[fill=white,align=left] at (cam3 -| kbr) {\stmt{BOOK isReachable true}\\
                                        \stmt{BOOK isVisible true} \\
                                        \stmt{BOOK isOn TABLE}};

         \node[fill=white,align=left ] at (cam3 -| kbh) (kb3) {\stmt{BOOK isReachable true}\\
                                         \stmt{BOOK isVisible true}};

         \node[fill=white,align=left] at (cam4 -| kbh) (kb4) {\stmt{HUMAN hasInHand BOOK}};

         \node[fill=white,align=left] at (cam5 -| kbr)  {\stmt{BOOK isIn BIN}};
         \node[fill=white,align=left] at (cam5 -| kbh) (kb5) {\stmt{BOOK isIn BIN}};
         ...
         \node[anchor=north, plan, robot] at (cam1.south -| probot) (pr1) {\bf TAKE\\BOOK\\TABLE};
         \node[anchor=north, plan, robot] at (cam2.south -| probot)  (pr2) {\bf PUT\_REACH\\BOOK\\TABLE} edge[<-] (pr1);
         \node[anchor=north, plan] at (cam3.south -| phuman) (ph1) {\bf TAKE\\BOOK\\TABLE} edge[<-] (pr2);
         \node[anchor=north, plan]  at (cam4.south -| phuman) (ph2) {\bf PUT\\BOOK\\BIN} edge[<-] (ph1);
     ...
     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         \node[anchor=north] at (ph1 -| ahuman) (wait1) {%
             \begin{tikzpicture}
                 \node[fill=white] (title) {\it wait for pick\\ {\tt BOOK} \it from {\tt TABLE}};
         ...
         \node[anchor=north] at (ph2 -| ahuman) (wait2) {%
             \begin{tikzpicture}
                 \node[fill=white] (title) {\it wait for put\\ {\tt BOOK} \it into {\tt BIN}};
...
\begin{frame}{``Cleaning the table''...}
    ...
    Example of a subplan: one book, accessible to the robot, to be
    handed over to the human and discarded.

     \includegraphics[width=0.8\paperwidth]{cleantable/manip_run_cam1.png}

\end{frame}
\begin{frame}<2,3,4,5>{``Cleaning the table''...}
\centering

    %\hspace{-2.5cm}\resizebox{!}{0.8\paperheight}{%
\hspace{-0.5cm}\resizebox{0.9\paperwidth}{!}{
    \begin{tikzpicture}[
        >=latex,
        every edge/.style={draw, ultra thick, ->},
        every node/.style={align=center},
        ...
        \node<1,2>[fill=white,align=left] at (cam1 -| kbr) (kb1) {\stmt{BOOK isVisible true}\\
                                                        \stmt{BOOK isReachable true}\\
                                                        \stmt{BOOK isOn TABLE}\\
                                                        \stmt{BIN isVisible true}\\
                                                        \stmt{BIN isReachable false}};

        \node<1,2>[fill=white,align=left] at (cam1 -| kbh) {\stmt{BOOK isVisible true}\\
                                                        \stmt{BOOK isReachable false}\\
                                                        \stmt{BOOK isOn TABLE}\\
                                                        \stmt{BIN isVisible true}\\
                                                        \stmt{BIN isReachable true}};

        \node<1,3>[fill=white,align=left] at (cam2 -| kbr) (kb2) {\stmt{ROBOT hasInHand BOOK}};


        \node<1,4>[fill=white,align=left] at (cam3 -| kbr) {\stmt{BOOK isReachable true}\\
                                            \stmt{BOOK isVisible true} \\
                                            \stmt{BOOK isOn TABLE}};

        \node<1,4>[fill=white,align=left ] at (cam3 -| kbh) (kb3) {\stmt{BOOK isReachable true}\\
                                             \stmt{BOOK isVisible true}};

        \node<1,5>[fill=white,align=left] at (cam4 -| kbh) (kb4) {\stmt{HUMAN hasInHand BOOK}};

        \node<1,5>[fill=white,align=left] at (cam5 -| kbr)  {\stmt{BOOK isIn BIN}};
        \node<1,5>[fill=white,align=left] at (cam5 -| kbh) (kb5) {\stmt{BOOK isIn BIN}};
        ...
        \node<1,2>[anchor=north, plan, robot] at (cam1.south -| probot) (pr1) {\bf TAKE\\BOOK\\TABLE};
        \node<1,3>[anchor=north, plan, robot] at (cam2.south -| probot)  (pr2) {\bf PUT\_REACH\\BOOK\\TABLE} edge[<-] (pr1);
        \node<1,4>[anchor=north, plan] at (cam3.south -| phuman) (ph1) {\bf TAKE\\BOOK\\TABLE} edge[<-] (pr2);
        \node<1,5>[anchor=north, plan]  at (cam4.south -| phuman) (ph2) {\bf PUT\\BOOK\\BIN} edge[<-] (ph1);
        ...
        \draw<1>[dotted, ->, in=45, out=180] (ae1.west) to (kb2);
...
\begin{frame}[label=ctrlstrategies]{Control strategies}
    ...
    \begin{itemize}
        ...
        \item not very agile, reacting to unexpected events hard
    \end{itemize}
...
\begin{frame}<1-7>[label=archi]{A symbolic control architecture}
    \begin{figure}
        \centering
    ...
    stmt/.style={align=center, font=\bf},
          ...
          \node at (0,0) (geom) {{\bf Situation Assessment} -- geometric \& temporal reasoning};
                ...
                %%% HATP
                \uncover<5->{
                \node at (-6, 2.5)[skill, fill=hriSec1!\dimming] (hatp) {{\bf Task planner}\\ \footnotesize human-aware};
                ...
                \path (lowlevel.east) edge [<-, bend right=60, looseness=1.9]
                node[label] {atomic\\actions} (shary.south east);
                }
  ...
  \only<7>{
      ...
      \node[stmt] at (domain) {isAvailable(?effector) $\wedge$ isA(?effector, Gripper)\\isOn(bottle, ?obj)};
...
\section{A glimpse at cognitive architectures}
...
\begin{frame}[plain]
    ...
    \centering
    \bf
    What is a \emph{cognitive} architecture?
}

    \only<2>{
    \highlightbox{Support modelling (human) social cognition\\\scriptsize(application to robots nice to have)}

        \vspace{2em}
        or...\\
        \vspace{1em}

        \highlightbox{Support thinking about robot programming \\in term of socio-cognitive skills}
    }
...
\imageframe[caption=2018 meta-meta-review by Kotseruba and Tsotsos]{cogarchs-kotseruba-tsotsos2018}

\begin{frame}{Three Interpretations of Cognitive Architectures}
    ...
    {\bf 3. Cognitive architectures as Methodology}
...
{
    \paper{Devin, Alami {\bf An implemented theory of mind to improve
    human-robot shared plans execution}, HRI, 2016}
...
\begin{frame}{3. Cognitive architectures as Methodology}    
...
\imageframe[color=white]{figs/islands4}

{
    \paper{Baxter, Lemaignan, Trafton {\bf Workshop on Cognitive Architectures
    for Social HRI} -- HRI 2016}
...
\begin{frame}{Additional bibliography}
\begin{thebibliography}{10}
\footnotesize
    \beamertemplatearticlebibitems
    \bibitem{kotseruba2018review}
    I. Kotseruba, J. K. Tsotsos
    \newblock \doublequoted{A Review of 40 Years in Cognitive Architecture
        ResearchCore Cognitive Abilities and Practical Applications}
    \newblock 2018
