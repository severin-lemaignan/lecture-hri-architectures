...
\begin{frame}{Task planning}
    ...
    \centering
        ...
        \includegraphics[width=0.9\linewidth]{cleantable/clean-table-situation}
    ...
    \resizebox{0.9\linewidth}{!}{%
        \begin{tikzpicture}[
            ...
            \node<2> at (h1) (hh1) {\bf TAKE\\WHITE\_TAPE\\TABLE};
            \node<2>[right=of hh1] (h2) {\bf PUT\\WHITE\_TAPE\\PINK\_BIN } edge (hh1);

            \node[robot, below=2.5 of h1] (r1) {\bf TAKE\\BLACK\_TAPE\\TABLE };
            \node[robot, right=of r1] (r2) {\bf PUT\_REACH\\BLACK\_TAPE\\TABLE } edge (r1);
            \node[robot, right=of r2] (r3) {\bf TAKE\\GREY\_TAPE\\TABLE } edge (r2);
            \node[robot, right=of r3] (r4) {\bf PUT\_REACH\\GREY\_TAPE\\TABLE} edge (r3);

            \node<2> at (r3 |- h1) (h3) {\bf TAKE\\BLACK\_TAPE\\TABLE} edge (h2) edge (r2);
            \node<2>[right=of h3] (h4) {\bf PUT\\BLACK\_TAPE\\PINK\_BIN} edge (h3);
            \node<2>[right=of h4] (h5) {\bf TAKE\\GREY\_TAPE\\TABLE} edge (h4) edge (r4);
            \node<2>[right=of h5] (h6) {\bf PUT\\GREY\_TAPE\\PINK\_BIN} edge (h5);
...
\imageframe[caption=2018 review by Kotseruba and Tsotsos]{cogarchs-kotseruba-tsotsos2018}
